...
2. Find the task in `docs/ROADMAP.md` and note its model tier ([H]/[C]/[Q]). If the tier is [H] and the current model isn't the highest-capability one in the "Model tiers" table of `BUILD-GUIDE.md` (at the **repo root**, not in `docs/`), tell me before continuing.
