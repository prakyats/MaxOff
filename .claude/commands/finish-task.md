...
---
Finish the current MaxOff unit (or single task).
...
4. Update the docs: tick the unit's tasks in `docs/ROADMAP.md`, and list the screens the unit added or changed in PROGRESS.md for the phase's phone walk (`/review-phase`). In `docs/PROGRESS.md`, update "Current state" and "Next task" (with its model tier), clear the handoff, add a session-log line, and record ideas, tech debt and gotchas. Update DATA-MODEL / WORKFLOWS / PERMISSIONS / ARCHITECTURE or add an ADR if anything changed.
...
6. Tell me the unit is done, summarize it in 3–5 lines, name the next unit and its model tier, and remind me to run `/clear` before `/start-task`.
