...
- A member edits their **own** name, phone and avatar (plain edit, audited). Role, job title and status are `team.manage` only. **Email is never self-edited**: it's the login identity, and changing it goes through the Owner (a transition function planned for 1.4; until then the Owner deactivates the old sign-in and invites the new address). `members.status` and its timestamps change only through transition functions (`app.protect_columns()` guard).
