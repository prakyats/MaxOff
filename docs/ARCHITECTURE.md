...
- **c. Bottom-nav tabs are roots, not history.** Back from any tab's root goes to the home tab (My Day for Staff, Today for Owner and Admin); back on the home root exits the app. **No "confirm exit".** (`core/ui/shell/tab-history`.) Pages opened from the More sheet (and Me) are roots too: the link backs out the sheet's history entry first (`closeOverlaysThen`: waits for the popstate, falls back after 300 ms, runs once, ignores a double tap), then pushes from home or replaces from another tab. The desktop sidebar keeps normal web history.
