...
`@sentry/nextjs`, initialised once per runtime from `src/core/observability` (`server.ts`, `edge.ts`, `client.ts` through `src/instrumentation.ts` and `src/instrumentation-client.ts`), errors only: `tracesSampleRate: 0`, no replay, no profiling. **The server and edge runtimes send with `core/observability/transport.ts` (platform `fetch`)**: the Node SDK's default `node:https` transport never completes on Workers under `nodejs_compat` (the flush hits its 2 s timeout and the event is lost), which the first staging diagnostic runs proved. Never set `debug: true` in a committed build: the SDK logger prints the raw, unscrubbed event message to Workers Logs. With no DSN the SDK is disabled, which is the local and CI state. Source maps are uploaded only when the deploy workflow provides `SENTRY_AUTH_TOKEN`, and an upload failure never fails a deploy.
