...
- `app → modules (index only) → core`, **plus a module's client components one file at a time** (`@/modules/<m>/components/<file>`, ADR-0011 amendment 2026-09-25). `core` never imports modules. No cycles. Files directly under `src/` (`proxy.ts`, `instrumentation.ts`) follow the `app` rules, and every file under `src/` must belong to a known element.
...
- Nothing outside a module imports its `data/`, `domain/` or `actions/`, and no other module imports its `components/`. `app/` may import a file from `components/` directly, and only a `"use client"` one (`tests/module-components.test.ts` checks every such import); server components go through `index.ts`, which exports **no client components**.
...
- **Route files and pages import client components from their own module file, never from a barrel that re-exports client components.** A barrel (`index.ts`) of client components is **not tree-shaken per route**: every client component it exports joins the bundle of any route that imports it, whether or not that route renders it. Measured in 1.5: `src/app/(auth)/login/page.tsx` imported `FormAlert` and `LoginForm` from `@/core/auth/components`, and the sign-in page therefore shipped `logout-confirm` too — **sonner plus radix-alert-dialog, 528 KB decompressed**, on a page with no toasts and no dialogs. Importing each form from its own file took `/login` from 18 requests and 1344 KB to 13 and 794 KB. A barrel is still the right thing for server-only helpers and types. **2.8 found it broken again**: the module barrels exported their client components, so `/my-day` shipped `LoginForm`, the attendance history (`@tanstack/react-table`) and both Approvals groups, and `/today`, `/my-day` and `/approvals` were byte-identical at 1441 KB. Since 2.8 a module's `index.ts` exports no client components, `core/auth/components` has no barrel, and routes import client components per file (lint allows `components/` from `app/` for exactly this). The same trap one level down: **a client component must not import a module that pulls a server-sized library for one constant.** `schemas.ts` files import zod, so a form's `maxLength` comes from the module's zod-free `domain/limits.ts`; `@/core/errors` re-exports `action()` (zod + server error reporting), so browser code imports `core/errors/codes`, `/result` and `/bulk` directly; `core/time/clock.ts` gives "now" without date-fns. **The enforcement is `pnpm budget`** (task 2.8, in `pnpm check` and CI): first-load JS per route from `next build`'s manifests against `bundle-budget.json`, failing the build when a budgeted route is over. `node scripts/bundle-budget.mjs --all` lists every page.
...
- **`src/proxy.ts`** runs `core/auth` `updateSession()` on every page request: refreshes the cookies and redirects from the JWT alone (no session → `/login?next=`; a session on the sign-in pages → `/`). **`/` is answered here too (2.7), because it is the installed app's `start_url`:** no session → `/login`; a session with a valid **home hint** → the role's home in one redirect, with no render first (`core/auth/home-hint.ts`). The hint is the httpOnly cookie `maxoff_home` = `<user id>|<home path>`, set at sign-in and set-password and refreshed with the day gate's daily pass (`submitDayChoice`, `issueDayPass`), so a role change reaches it within a day; the Owner, never gated, refreshes it at sign-in (the Owner's role cannot change). **It is a routing hint, never a decision:** the page it leads to runs `requireMember()` as always, so it carries no MAC, and it is bound to the user id so another person on the device never inherits it. Missing, someone else's or not a home → the request falls through to `src/app/page.tsx`, which reads the member and redirects; the proxy never answers `/` with `/`, so a bad hint costs one hop and cannot loop. Sign-in and set-password redirect straight to `homeFor(role)` (or a safe `next`), never through `/`. (Owner decision 2026-09-25: a cookie, not a custom access token hook — a hook failure would block every sign-in, a large blast radius for a routing hint whose worst failure here is one extra redirect.) Static assets, `sw.js` and the manifest are outside its matcher; `/offline`, `/auth/*`, `/api/*` and the sign-in pages pass through it without a session (`core/auth/paths.ts`). OpenNext bundles it as Node middleware.
...
- **A transient failure is never "signed out"** (2.6, owner decision 2026-09-24; `core/auth/session-errors.ts`, unit-tested both ways). `classifySessionError()` calls a `getClaims()` failure **transient** when GoTrue could not be reached or did not answer properly (auth-js's `AuthRetryableFetchError`: network errors, timeouts, 5xx; a 429; any non-auth exception) and **invalid** when GoTrue looked at the session and refused it (no session, an expired token whose refresh was refused, a bad or revoked refresh token, an invalid JWT). Transient: the proxy passes the request through untouched and `getSessionState()` throws `SessionUnavailableError`, whose `digest` (`SESSION_UNAVAILABLE_DIGEST`, the one thing Next forwards from a server error in production) makes the error boundary say **"Can't reach the server. You're still signed in."** with Try again, reported to Sentry. Invalid: `/login`, as before. An expired session never lands on the error page. Before 2.6 both readers treated every `getClaims()` error as "no session", which would have signed the whole team out during a GoTrue blip.
...
`public/manifest.webmanifest`, icons and iPhone launch screens (`public/icons/`, rendered from `icon.svg` and `core/ui/pwa/launch-screens.json` by `scripts/generate-icons.mjs`) and a plain-JS service worker `public/sw.js` (task 0.5). The worker is a **minimal offline shell**: it precaches `/offline`, serves it when a navigation fails without a connection, caches hashed `/_next/static` and `/icons` assets cache-first, and never touches `/api`, non-GET requests or other origins, so Supabase and server actions are always live. It is registered by `<RegisterServiceWorker />` in the root layout **only in production builds** (`next dev` and Playwright never see it). Push handlers join the same file in 5.1 (ADR-0009). `src/core/ui/pwa/pwa-files.test.ts` keeps the manifest, icons, headers and worker consistent with the tokens. **The launch (2.7):** the splash, the first frame and the intro draw one mark (`icon.svg`'s rounded red square) at one size (`launch-screens.json` `markSize`, 144 CSS px) in one place (centred in the web view) on the dark launch background. The **maskable** icon puts that whole mark at 60% of an opaque dark canvas (its corners just inside the 40%-radius safe circle), because Android draws the maskable icon at a fixed size on the splash and a full-bleed red square read as an oversized red disc; the any-purpose icons stay the plain mark. **iPhones** get an `apple-touch-startup-image` per current screen size (`appleWebApp.startupImage` in the root layout, portrait, the mark centred below the status bar), or an installed launch starts on white. **The intro** (`core/ui/pwa/launch-intro.tsx`, styles in `globals.css`) is a cover drawn with that mark in the first frame that fades out (300 ms, a 0.96 scale settle; reduced motion: the fade only), `pointer-events: none`, waiting for nothing, so it can never add delay. A blocking `<head>` script marks `<html data-launch>` only for an installed window's first document (`sessionStorage`), so a reload, a browser tab, a return from background and a client-side navigation never see it; it plays on a signed-out cold start (/login) too. `scripts/measure-launch.mjs` measures a launch (fresh context, signed in, phone size; `--no-hint` for the pre-2.7 path); numbers in PROGRESS. `e2e/launch.spec.ts` covers the redirect, the intro and the touch rules. **Every screen is designed for the phone first** (PRODUCT §1, task 1.5): mobile is its own layout for every role, not a narrow desktop one, and the desktop layout is the second one the same data gets. §14.1 is the standard; the shell rules in `globals.css` and the composites are what every screen inherits.
...
- **Editing is explicit: read-only by default, explicit edit, explicit save, confirm what changed, guard unsaved work.** A screen that shows someone's own identity — or any record — renders as plain values with a **pencil Edit** affordance (icon plus label, 44px). Edit mode puts **Save and Cancel in a sticky bar**, with Save disabled until something actually changed. The confirmation names the change ("Your name will change from X to Y"), never a generic "Are you sure?". Leaving with unsaved changes warns first, **including the back gesture**, which is history-aware. Saving gives clear "Saved" feedback. Live fields that save silently are wrong here: changes to identity and records should feel deliberate. **Built once as a shared component (task 2.9) and copied, not reinvented per screen** — the client screens in 3.4 are the first consumers. **As built (2.9):** `EditableRecord` (`core/ui/composites/editable-record.tsx`) takes `title`, `subject` (`"self"` or the person's name, for the copy), `fields` (name, label, `noun` for the sentence, value, input props) and `onSave` (the server action, returning a `Result`). The baseline for "changed" is taken when Edit is tapped, so a refresh on return cannot move it. The confirmation's lines come from `core/ui/edit/changes.ts` (`diffFields`, `describeChange`: "Your name will change from X to Y", "… will be set to Y", "… will be removed"; whitespace alone is no change). A dialog that edits a record uses the same confirmation (`ChangeList`): `EditMemberDialog` since 2.9 ("Ravi's role will change from Staff to Admin"). Unsaved changes are guarded by `useLeaveGuard` (`core/ui/edit/`): in-app links are held in the capture phase on `window` and replayed after Discard, a reload gets the browser's `beforeunload` prompt, the Log out confirmation warns (`anyEditDirty()`), and refresh on return waits. The sticky bar exists only in edit mode, so it never hangs over the rest of a page.
...
**On a phone the installed app behaves like a native Android or iOS app, never like a website that remembers clicks.** This is a permanent rule (CLAUDE.md engineering rule 11), not a preference: every new screen, overlay and view control is built to it and proved by an installed-mode back-gesture spec at 375px and 430px (Definition of Done). Items a–h apply in the installed app; **d, a (overlays), i and k apply in every browser too.**

- **a. Back closes the topmost layer first, one layer per back:** keyboard → menu or popover → confirm or dialog → sheet → then the screen. Overlays register with `core/ui/overlay/overlay-history` through the `core/ui/primitives` roots (lint-enforced, §14.1) — `Sheet`, `Dialog`, `AlertDialog`, `DropdownMenu` and `Select`; a sheet that hands off to a confirm closes itself, so back closes the confirm and leaves the screen.
- **b. History holds only real drill-down:** list → detail → sub-detail. Back goes up exactly one level.
- **c. Bottom-nav tabs are roots, not history.** Back from any tab's root goes to the home tab (My Day for Staff, Today for Owner and Admin); back on the home root exits the app. **No "confirm exit".** (`core/ui/shell/tab-history`.) Pages opened from the More sheet (and Me) are roots too: the link backs out the sheet's history entry first (`closeOverlaysThen`: backs out **only when the controller owns the top entry**, so the popstate is certain and it waits for it with no timer; runs once; ignores a double tap), then pushes from home or replaces from another tab. **The overlay marker is guarded against Next rewriting `history.state`** (2.6): Next's history updater replaces the current entry's state without custom keys after any navigation, refresh or completed server action, so the controller wraps `replaceState` and carries its marker over — an entry once marked stays marked until a real navigation pushes a new one. The desktop sidebar keeps normal web history.
...
- **e. Gates and one-time screens are never in the back stack:** login, set-password, the day gate, recovery, and a form after it is submitted (the result replaces the form). A server action's `redirect()` pushes by default, so these pass `RedirectType.replace`; a redirect started from inside an overlay (Log out's confirmation) backs out the overlay's entry first, or the replace would land on the overlay's entry and leave the page under it.
- **f. Back on a form with unsaved changes asks "Discard changes?"** (Keep editing / Discard) and never loses work silently. **Built in 2.9:** edit mode is a history layer (`useOverlayHistory`): one back with nothing changed leaves edit mode; with changes it opens "Discard changes?", and one back on that dialog keeps editing (it is the top layer, §14.2 a). Save, Cancel and Discard back the layer's entry out (`closeOverlaysThen`), so none leaves a back press that lands on the same page. `e2e/edit-pattern.spec.ts` proves it installed at 375 and 430 px.
- **g. Returning to a list restores its scroll position and filters; each tab root keeps its own scroll.** *Built in 2.7b:* every tab move of `tab-history` (bottom bar and More sheet) saves the scroll of the tab it leaves and puts back the scroll of the tab it reaches (`core/ui/shell/tab-scroll.ts`, installed only, module state: a reload starts at the top). The place is held for 1 s after arrival, because the page may still be streaming in and Next scrolls a new page to the top on the commit that shows it; any touch, wheel or key hands the scroll back at once. Back from a drill-down is a history move, restored by the browser and Next.
...
- **i. Touch feel:** every tap shows a pressed state within 100 ms, and navigation shows a pending state; no double-tap zoom delay (`touch-action: manipulation`); no tap-highlight flash; no browser overscroll bounce leaking out of the app shell (`overscroll-behavior`); bottom sheets close by swiping down; a long-press on a control does not select text. *Built in 2.7 (`globals.css`, checked by `pwa-files.test.ts` and `launch.spec.ts`):* `-webkit-tap-highlight-color: transparent` on `html`; `overscroll-behavior-y: none` on `html` and `body`; `user-select: none` and `-webkit-touch-callout: none` on buttons, links, labels, summaries and the button-like roles (text content stays selectable). Still open: swipe-down on sheets, a pending state on ordinary `Link`s, the measured 100 ms pressed state (PROGRESS, Ideas).
  - **Zoom is locked only where system text size applies** (2.7b, owner decision 2026-09-25). The installed app does not pinch-zoom or double-tap-zoom; in exchange it follows the phone's own text size, and that trade is only fair where the text size actually reaches the app. Android's Chrome scales the app's text with the system font size, so an installed Android app is locked: `STANDALONE_SCRIPT` (`core/ui/pwa/standalone.ts`, in `<head>` on every document) appends a second viewport meta with `maximum-scale=1, user-scalable=no` after Next's (the last one wins; React's own element is never touched) and sets `html[data-zoom-lock]`, which gives `body` `touch-action: pan-x pan-y`. **iOS does not pass its text size to a web app** unless the app opts in, so an installed iPhone app keeps pinch-zoom as the accessibility safety valve (`navigator.standalone` exists only on iOS, so it is the check); the opt-in, a check at iOS larger text and then the iOS lock are in PROGRESS Ideas, tied to 6.6. A browser tab always zooms. Inputs stay at 16px so iOS never zooms on focus (§14.1). **Every screen survives large system text:** at 130% and 200% nothing scrolls sideways or sits past the edge, and a line an ellipsis cuts short keeps at least 64px (`e2e/mobile.spec.ts`, root font scaled as the stand-in); card and list rows wrap their trailing part under the title rather than squeezing it (`CARD_ROW_TITLE` / `CARD_ROW_TRAILING` in `row-metrics.ts`), page titles take two lines before they truncate.
  - **Pull-to-refresh is off on purpose** (2.7: a pull that reloads loses what the person was doing). Freshness comes from **refresh on return** (2.7b, `core/ui/shell/refresh-on-return.ts`, mounted in the `(app)` layout) and from Realtime in 5.1: when the app becomes visible again or its window regains focus, `router.refresh()` fetches the server data in place (lists, counts, the Approvals badge), with no reload and no splash, keeping open sheets, typed input and scroll. At most once every 30 s, and never while an approval is inside its Undo window (`anySendWaiting()` in `delayed-sends.ts`), so it cannot race the delayed send. The layouts' own checks run as on any navigation, so a return after midnight IST meets the day gate.
- **j. Motion:** a drill-down slides in and back slides out; tab switches do not slide (View Transitions where supported), always respecting `prefers-reduced-motion`. *Built in 2.7b:* **installed app at phone width only** (standalone and below 768px; a browser tab and the desktop behave like a website and never slide, owner decision 2026-09-25), never with reduced motion. A partial slide: 30% offset with a fade, 260 ms, decelerate easing; back reverses it (`--nav-slide-*` in `globals.css`). `RouteTransition` (`core/ui/motion/route-transition.tsx`, in the `(app)` layout) wraps the page in React's `<ViewTransition>` **keyed by the path** (a `template.tsx` would not remount for `/people` → `/people/[id]`, the same segment; a query-only change never remounts). **The tabs of one screen share one key** (`routeKey`, `core/ui/motion/route-key.ts`: `/leave` + `/leave/attendance`, `/people/[id]` + `/attendance`), and their header and tab bar live in the screen's layout (`leave/layout.tsx`, `people/[id]/layout.tsx`), so a tab switch swaps only the view: the header and tabs are never rebuilt or replaced by a skeleton (`motion.spec.ts` checks element identity and the skeletons drawn in their band). **A new screen with tab routes adds its entry to `routeKey` and draws its header and tabs in a layout.** A new key remounts everything under `(app)`, nested layouts included with `enter`/`exit` only for the `nav-forward` and `nav-back` types, so tabs, view controls, refreshes, streamed reveals and the **system back gesture** (owner decision: Android's predictive back covers it) change the page with no motion. **Drill-down links carry the type:** `DrillLink` (`core/ui/composites/drill-link.tsx`) and `OverlayLink` send `nav-forward`; `BackLink` sends `nav-back`, through its replace, and through `slideBack` (`core/ui/motion/slide.ts`) for its `router.back()`: a history move carries no type, and React renders the transition Next starts in `popstate` synchronously, so no listener can type it (both orders tried); `slideBack` starts that one view transition by hand and names the page with the same class through `html[data-nav-slide]`. The top bar and bottom bar are pinned above the moving page and the root is not captured. **A new drill-down link uses `DrillLink` (or `OverlayLink` from inside an overlay)**, a new drill-down screen's header uses `PageHeader` `back`. Proved by `e2e/motion.spec.ts`, which wraps `document.startViewTransition` and records the types and the animations that ran.
- **k. Every drill-down screen has an on-screen back control in its header; an installed iOS app has no system back gesture.** `PageHeader` `back={{ href: parent, label }}` renders `BackLink` (`core/ui/composites/back-link.tsx`, 2.4): with an entry of the app beneath (the Navigation API's `currentEntry.index > 0`) it goes back, adding nothing; opened directly (a deep link, a refresh in a new window, a browser without the API) it goes to the parent with **replace**, so it never leaves the app and never adds a step. It is a real link to the parent, so it works before hydration.
- **l. Before hydration, taps make the same moves** (task 2.8). Until React hydrates (measured ~0.8–1.0 s on /today and /my-day at 4× CPU throttling), every enhanced control is its server-rendered `<a>`, and a tap was a plain push: the back control went *forward* to the parent, and view controls and installed tabs stacked entries. `PRE_HYDRATION_SCRIPT` (`core/ui/navigation/pre-hydration.ts`), inline in `<head>` on every document, listens for clicks and makes the app's own move: `[data-slot=page-back]` goes back or replaces with the parent (`backMove`), `[data-view-link]` replaces (`viewMove`), an installed `[data-tab]` pushes, replaces or goes back (`tabMove`, reading the entry beneath through the Navigation API). No slide before hydration. **The hand-over is per link, not per document:** `BackLink`, `ViewLink` and the bar's tabs mark themselves `data-live` with a callback ref (`markLive`, `core/ui/navigation/attributes.ts`) in the commit that hydrates them, and the script leaves a live link to the app. (A document-wide mark from the root layout was tried first and handed over too early: the root hydrates before the streamed shell and page, so a back control in a page still streaming would have fallen through as a push; CI caught it through the e2e helper.) The rules are **one set of functions** (`core/ui/navigation/moves.ts`, used by `BackLink`, `ViewLink` and `tab-history`) that the script writes out in ES5; `move-cases.ts` is **one table of cases** run against both (`moves.test.ts`, `pre-hydration.test.ts`), so the two cannot drift. `e2e/pre-hydration.spec.ts` holds the page's script chunks forever and proves each move at 375 and 430 px, and that hydrated links are live.
...
**Deterministic by construction (2.6).** A red run has to mean something, so the suite removes its own sources of noise instead of allowing for them: **no timeout is raised and no retry is added to make a test pass.** (1) `e2e/global-setup.ts` waits for the stack to be *ready* (GoTrue answers, PostgREST reaches Postgres, Mailpit's API is up) before the build starts, because `db:reset` restarts the auth container and the database; a stack that never comes fails in two minutes with the reason. (2) `e2e/warm.setup.ts`, last in the serial `setup` project, requests every route once as the Owner: `next start` renders each route for the first time on its first request (about 0.5 s alone, 1.5–4 s when dozens of first renders compete the moment the parallel projects start), and that storm was every "cold-start" timeout 2.2 and 2.3 met. (3) The `setup` project's saved sessions are shared by every worker, and they are gated again at **midnight IST**: the global teardown names a run that crossed it, so a cascade of Admin and Staff failures is read as the day change, not the code. (4) Mailpit is shared and survives resets, so `latestEmailTo(to, since)` accepts only a message newer than a timestamp taken before the request; nothing wipes the mailbox. (5) Every spec that creates people or list entries removes them first (`removeFixturePerson()`, `removeJobTitles()`) and the recovery flow puts the seeded password back, so **the whole suite passes repeatedly after one `db:reset`**. (6) `trace: retain-on-failure`, so a red run is diagnosable without re-running it; `retries` is 0 everywhere; local `workers` are capped at 4 (a resource setting for a 16 GB laptop beside the Docker VM, not an allowance: a stall at four workers is a real cause). Proved on 2026-09-25: five full runs in a row after one `db:reset`, 285 passed each.
...
**The browser SDK is not in the first load** (task 2.8): it was the largest thing parsed before hydration on every screen. `instrumentation-client.ts` starts a small early reporter (`core/observability/early.ts`) that queues `error` and `unhandledrejection` events (at most 20) from the first script on; the SDK (`client-sdk.ts`) is imported once the page is idle after `load` (at most 3 s later), **or at once on the first error**, and the queue is replayed through it (`mechanism.handled: false`, so the scrubber still applies) before the early listeners step aside for the SDK's own. Boundary captures and `<SentryUser>` go through the same reporter (`core/observability/client.ts`). With no DSN nothing listens and the SDK never loads. What is lost: an error on a page closed before the SDK chunk arrives. **On the server the Node SDK stays** (`@sentry/nextjs`): measured in 2.8 it is ~0.18 s of a ~0.65 s route-independent Worker cold start locally (4.3 MB of the Worker); moving the Worker to `@sentry/cloudflare` is task 6.5b [H] (owner decision 2026-09-25: its own ADR, a scrubbing-parity test, and only if staging gains ≥ ~250 ms).
