...
`next.config.ts` (`headers()`) puts the same headers on every rendered response: `X-Frame-Options: DENY` and `Content-Security-Policy: frame-ancestors 'none'` (the app is never embedded), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, a `Permissions-Policy` that denies camera, microphone and geolocation, and HSTS for two years with subdomains. Static assets get their cache headers from `public/_headers`. `e2e/production.spec.ts` asserts the exact values against a real build. A full CSP with script sources is deferred until the app has its final set of inline scripts (Next, next-themes); when it arrives, add it here and in that spec.
