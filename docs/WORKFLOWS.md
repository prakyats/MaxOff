...
- **Logout:** `session_logout()` writes `session_events(kind = logout)` immediately (this device only; other signed-in devices stay in), then `app.attendance_logout()` stamps `attendance_days.last_logout_at` (2.1, see the 2.1 bullet below), then the auth session is ended.
...
- **Nightly (00:00 IST, after the absent check):** days with a login and no logout get `logout_not_recorded = true`. No time is made up. **A logout after midnight clears it** (2.5, owner decision 2026-09-25): the flag means "no logout was recorded", and `app.attendance_logout()` landing on yesterday's day records one, so that write carries `last_logout_at` and the cleared flag; nothing else moves.
...
- **Settled in 2.2 (owner decisions, 2026-09-23):**
  ...
  - **Two devices at once.** `attendance_touch()` is serialised per member (an advisory lock taken before anything else), so a phone and a laptop opening the app at the same instant produce exactly one day and **one** `session_events(login)` from the touch. A real sign-in on each device still writes its own login row through `session_login()`: those are two real events.
...
| `logout_not_recorded` | pg_cron | 00:00 (18:30 UTC, the minute after absent_check) | Flags days with a login and no logout. Built in 2.5 |
