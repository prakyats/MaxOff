...
select ok(
  not has_function_privilege('authenticated', 'public.bootstrap_owner(uuid, text, text, text)', 'execute')
  and has_function_privilege('service_role', 'public.bootstrap_owner(uuid, text, text, text)', 'execute'),
  'bootstrap_owner is service_role only');

-- bootstrap_owner: validation on an empty team -------------------------------------------------
...
select throws_ok(
  $$ select public.bootstrap_owner(pg_temp.fx('nobody'), 'nobody@example.com', 'Nobody', 'Boot Org') $$,
  'P0001', 'NOT_FOUND', 'the auth user must exist first');
select throws_ok(
  $$ select public.bootstrap_owner(pg_temp.fx('owner'), 'not-an-email', 'Test Owner', 'Boot Org') $$,
  'P0001', 'VALIDATION', 'the email must look like one');
select throws_ok(
  $$ select public.bootstrap_owner(pg_temp.fx('owner'), 'owner@example.com', '  ', 'Boot Org') $$,
  'P0001', 'VALIDATION', 'the name is required');
select throws_ok(
  $$ select public.bootstrap_owner(pg_temp.fx('owner'), 'other@example.com', 'Test Owner', 'Boot Org') $$,
  'P0001', 'VALIDATION', 'the member email must be the auth user''s email');
select throws_ok(
  $$ select public.bootstrap_owner(pg_temp.fx('owner'), 'owner@example.com', 'Test Owner', null) $$,
  'P0001', 'VALIDATION', 'the organization name is required when none exists');

-- bootstrap_owner: creates the organization when there is none ---------------------------------
savepoint fresh_install;
select is(public.bootstrap_owner(pg_temp.fx('owner'), '  Owner@Example.com ', ' Test Owner ', ' Boot Org '), pg_temp.fx('owner'),
  'bootstrap_owner returns the member id');
...
select results_eq(
  $$ select full_name, email, role::text, status::text, joined_at is not null, deactivated_at is null,
       ...
       from public.members where id = pg_temp.fx('owner') $$,
  $$ values ('Test Owner', 'owner@example.com', 'owner', 'active', true, true, true) $$,
  'the Owner is active, joined, lower-cased and trimmed, in the new organization');
select is(
  (select count(*) from public.activity_log
     where entity = 'members' and entity_id = pg_temp.fx('owner') and action = 'insert' and actor_id is null),
  1::bigint, 'the bootstrap insert is audited as a system action');
select throws_ok(
  $$ select public.bootstrap_owner(pg_temp.fx('admin'), 'admin@example.com', 'Second', 'Boot Org') $$,
  'P0001', 'CONFLICT', 'a second call is refused once a member exists');
rollback to savepoint fresh_install;

-- bootstrap_owner: uses the existing single organization ---------------------------------------
insert into public.organizations (name) values ('Test Org');
insert into fx select 'org', id from public.organizations limit 1;
select is(public.bootstrap_owner(pg_temp.fx('owner'), 'owner@example.com', 'Test Owner', null), pg_temp.fx('owner'),
  'with one organization, bootstrap_owner needs no organization name');
select is((select count(*) from public.organizations), 1::bigint, 'no second organization was created');
select is((select org_id from public.members where id = pg_temp.fx('owner')), pg_temp.fx('org'),
  'the Owner belongs to the existing organization');
...
select throws_ok(
  $$ select public.bootstrap_owner(pg_temp.fx('nobody'), 'x@example.com', 'X', 'Y') $$,
  '42501', null, 'not even the Owner may call bootstrap_owner through the API');
...
-- session_login ------------------------------------------------------------------------------
select pg_temp.as_member('owner');
select isnt(public.session_login('Mozilla/5.0 test', 'abc123'), null, 'the Owner records a login');
select results_eq(
  $$ select member_id, kind, user_agent, ip_hash from public.session_events $$,
  $$ values (pg_temp.fx('owner'), 'login', 'Mozilla/5.0 test', 'abc123') $$,
...
select pg_temp.as_member('owner');
select is((select count(*) from public.session_events), 6::bigint, 'the Owner (attendance.view_all) sees every event');
select pg_temp.as_member('deactivated');
